    ...
    Q.load(["wind.png","song.mp3","3.png","1_animation.png","2_animation.png","4_animation.png"], function() {
        ...
        fbRef.on("value", function(snapshot) {
            ...
            for(key in gotCars){
                if(!cars[key] && stageAccess != null){
                    ...
                    for(ik in cars){
                        cars[ik].p.x = newX;
                        cars[ik].p.y = newY + step * iter;
                        if(iter == 1){newX -= step*iter;iter = 0;newY = 870;}else{iter++;}
                    }
                ...
                console.log("p.passedControlPoints" + p.passedControlPoints);
        ...
        Q.Sprite.extend("Car", {
            init: function(p) {
                ...
                var model = Math.floor((Math.random() * 4) + 1);
                        ...
                        if(collided.obj.p.counter){
                            ...
                            if(collided.obj.p.finish){
                                if(p.passedControlPoints == true){
                                    ...
                                    raceStarted = false;
                                    var container = stageAccess.insert(new Q.UI.Container({fill: "gray",border: 5,shadow: 10,shadowColor: "rgba(0,0,0,0.5)",y: 100,x: 1000}));
        ...
        Q.scene("Race",new Q.Scene(function(stage) {
            ...
            stageAccess.insert(new Q.Counter({w:500,h:10,x:1380,y:420}));// tree
            stageAccess.insert(new Q.Counter({w:10,h:500,x:700,y:820,angle:350,finish:true}));// tree
            
            
            stageAccess.insert(new Q.Wind({x:184,y:850}));// wind

            stage.insert(new Q.UI.Button({
                label: "Press to START RACE!",
                y: 70,
                x: 1750,
                fill: "#990000",
                border: 5,
            ...
            var counter = function(){
                var container = stage.insert(new Q.UI.Container({fill: "gray",border: 5,shadow: 10,shadowColor: "rgba(0,0,0,0.5)",y: 100,x: 1000}));
